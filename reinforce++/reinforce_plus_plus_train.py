"""
Simple REINFORCE++ Training for LLM.
"""
...
def masked_var(values, mask, unbiased=True):
    ...
    mean = masked_mean(values, mask, dim = None) # dim = None for overall mean (batch-level)
    centered_values = values - mean
    variance = masked_mean(centered_values**2, mask, dim = None) # dim = None for overall variance (batch-level)
    if unbiased:
        # Bessel's Correction
        mask_sum = mask.sum()
        if mask_sum == 0:
...
def masked_whiten(values, mask, shift_mean=True):
    ...
    """
    mean, var = masked_mean(values, mask, dim = None), masked_var(values, mask)
...
class Reinforce_Plus_Plus_Trainer:
    ...
    # ------------------------------------------------------------------
    # Step 2 · Make experience: reinforce_plus_plus_train advantage (no critic)
    # ------------------------------------------------------------------
    @torch.no_grad()
    def make_experience(
        self,
        ...
        gamma=args.gamma
        with torch.no_grad():
            returns = torch.zeros_like(rewards_kl)
            running_return = 0

            for t in reversed(range(rewards_kl.shape[1])):
                running_return = rewards_kl[:, t] + gamma * running_return
                returns[:, t] = running_return
                # Reset after EOS, advoid PAD token
                running_return = running_return * action_mask[:, t]

            advantages = masked_whiten(returns, action_mask)
    ...
    # ------------------------------------------------------------------
    # Step 3 · REINFORCE++ update: batch_size -> mini_batch_size -> micro_batch_size (grad accumulation)
    # ------------------------------------------------------------------
    def reinforce_plus_plus_update(self, experience: dict) -> dict:
        """
        ...
        for mini_start in range(0, N, args.ppo_mini_batch_size):
            ...
            for micro_start in range(0, len(mini_idx), args.ppo_micro_batch_size_per_gpu):
                ...
                # ──────────────────────────── reinforce++ loss begin ──────────────────────────
                # Note: the loss function is the same as PPO.
    ...
    # ------------------------------------------------------------------
    # Evaluation (greedy generation, no REINFORCE++ update)
    # ------------------------------------------------------------------
    @torch.no_grad()
    ...
    def train(self):
        ...
        print(f"Start REINFORCE++ training for {args.num_train_epochs} epoch(s).")
        ...
        for epoch in range(1, args.num_train_epochs + 1):
            ...
            for batch in self.dataloader:
                ...
                update_stats = self.reinforce_plus_plus_update(experience)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Simple REINFORCE++ training for LLM")
    # paths
    parser.add_argument("--model_path", type=str, required=True, help="Path to pretrained model.")
    parser.add_argument("--train_json", type=str, required=True, help="Path to training json file.")
    parser.add_argument("--output_dir", type=str, default="output_dir/reinforce_plus_plus")
    # generation
...
def main():
    args = parse_args()
    trainer = Reinforce_Plus_Plus_Trainer(args)
    trainer.train()
