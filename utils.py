...
def get_output_dir_structure(output_dirs: str) -> tuple[str, str, str]:
    """
    Create a new directory structure for output organization.
    Returns (base_dir, cache_dir, runs_dir) where:
    - base_dir: timestamped directory under output_dirs
    - cache_dir: cache subdirectory
    - runs_dir: runs subdirectory
    """
    now = datetime.now()
    timestamp = now.strftime("%Y-%m-%d_%H-%M-%S_%f")
    
    # Create base directory with timestamp
    base_dir = f"{output_dirs}/{timestamp}"
    os.makedirs(base_dir, exist_ok=True)
    
    # Create cache and runs subdirectories
    cache_dir = f"{base_dir}/cache"
    runs_dir = f"{base_dir}/runs"
    os.makedirs(cache_dir, exist_ok=True)
    os.makedirs(runs_dir, exist_ok=True)
    
    return base_dir, cache_dir, runs_dir
