"""
Simple RLOO (REINFORCE Leave-One-Out) Training for LLM.

RLOO computes per-response advantages using a leave-one-out baseline:
    advantage_i = r_i - mean_{j != i}(r_j)
               = r_i * G/(G-1) - sum(r_group)/(G-1)

This is an unbiased baseline that reduces variance without requiring
...
def compute_rloo_advantages(rewards: torch.Tensor, group_ids: torch.Tensor) -> torch.Tensor:
    """
    Compute RLOO (Leave-One-Out) advantages.

    For each response i in group g with G members:
        baseline_i = mean of rewards of all other G-1 responses in group g
                   = (sum_g - r_i) / (G - 1)
        advantage_i = r_i - baseline_i
                    = r_i * G/(G-1) - sum_g/(G-1)

    When G == 1, advantage is set to 0 (leave-one-out is undefined).

    Args:
        rewards:   (N,) scalar reward per response
        group_ids: (N,) integer group id for each response (same id = same prompt)

    Returns:
        advantages: (N,) leave-one-out advantage per response
    """
    advantages = torch.zeros_like(rewards)
    for gid in group_ids.unique():
        mask = group_ids == gid
        group_rewards = rewards[mask]           # (G,)
        G = group_rewards.shape[0]
        if G == 1:
            # Cannot compute leave-one-out with a single sample; advantage stays 0
            advantages[mask] = torch.zeros_like(group_rewards)
        else:
            group_sum = group_rewards.sum()
            # advantage_i = r_i * G/(G-1) - group_sum/(G-1)
            advantages[mask] = group_rewards * G / (G - 1) - group_sum / (G - 1)
    return advantages
...
class RLOOTrainer:
    ...
    def rollout(
        ...
        *,
        ...
        return {
            ...
            "prompt_len":     prompt_len,        # (B*G, )
            "group_ids":      group_ids,         # (B*G, )
        }

    # ------------------------------------------------------------------
    # Step 2 · Make experience: RLOO leave-one-out advantage (no critic)
    # ------------------------------------------------------------------
    @torch.no_grad()
    def make_experience(
        self,
        ...
        # ── KL per sequence (for logging) ──
        kl_per_seq = compute_approx_kl(old_log_probs, ref_log_probs, action_mask, kl_estimator="k3")  # (N,)

        # ── KL-shaped reward: place dense KL penalty at every token,
        #    and add the scalar outcome reward at the last valid token ──
        # (same as PPO: rewards_kl = -kl_coef * KL_dense, then += outcome at EOS)
        kl_dense = (old_log_probs - ref_log_probs) * action_mask            # (N, S-1)
        rewards_shaped = -args.kl_coef * kl_dense                           # (N, S-1)
        for i in range(sequences.shape[0]):
            last_idx = (action_mask[i].nonzero(as_tuple=False)[-1].item()
                        if action_mask[i].any() else 0)
            rewards_shaped[i, last_idx] += rewards[i]

        # ── shaped scalar per response: sum over tokens ──
        shaped_scores = rewards_shaped.sum(dim=-1)                           # (N,)

        # ── RLOO leave-one-out advantage (computed on shaped scores) ──
        # For each response i in group g (size G):
        #   baseline_i = mean of shaped_scores of all other G-1 responses
        #   advantage_i = shaped_score_i - baseline_i
        advantages = compute_rloo_advantages(shaped_scores, group_ids)
    ...
    # ------------------------------------------------------------------
    # Step 3 · RLOO update: REINFORCE policy gradient + KL penalty (no critic)
    # ------------------------------------------------------------------
    def rloo_update(self, experience: dict) -> dict:
        """
        RLOO policy gradient update (no PPO clipping).

        Unlike GRPO which uses PPO-clip, RLOO uses plain REINFORCE:
            loss = -mean[ log_prob(a|s) * advantage ]

        The leave-one-out baseline already provides variance reduction
        without requiring importance-ratio clipping.
        ...
        for mini_start in range(0, N, args.ppo_mini_batch_size):
            ...
            for micro_start in range(0, len(mini_idx), args.ppo_micro_batch_size_per_gpu):
                ...
                # ── RLOO REINFORCE policy gradient loss (no clipping) ──
                # KL penalty is already baked into advantages via reward shaping in make_experience
                # broadcast per-response advantage → per-token
                token_advantages = advantages.unsqueeze(1).expand_as(new_log_probs) * action_mask

                # loss = -E[log_prob * advantage]  (plain REINFORCE, no clipping)
                actor_loss = -masked_mean(new_log_probs * token_advantages, action_mask).mean()
                ...
                loss = (actor_loss + args.entropy_coef * entropy_loss) / actual_accum
                loss.backward()
    ...
    def train(self):
        ...
        print(f"Start RLOO training for {args.num_train_epochs} epoch(s).")
        ...
        for epoch in range(1, args.num_train_epochs + 1):
            ...
            for batch in self.dataloader:
                ...
                # ── Step 2: Make experience (RLOO leave-one-out advantages) ──
                experience = self.make_experience(rollout_data, rewards)

                # ── Step 3: RLOO update ──
                update_stats = self.rloo_update(experience)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Simple RLOO training for LLM")
    # paths
    parser.add_argument("--model_path", type=str, required=True, help="Path to pretrained model.")
    parser.add_argument("--train_json", type=str, required=True, help="Path to training json file.")
    parser.add_argument("--output_dir", type=str, default="output_dir/rloo")
    # generation
    ...
    parser.add_argument("--num_answers_per_question", type=int, default=4,
                        help="Number of responses generated per prompt (group size G >= 2 for RLOO). "
    ...
    parser.add_argument("--entropy_coef", type=float, default=0, help="Entropy loss coefficient. Default 0.")
    # optimizer
