"""
Simple DAPO (Decoupled Clip and Dynamic sAmpling Policy Optimization ) Training for LLM.
"""
...
def dapo_length_penalty(rewards: torch.Tensor, 
                        response_len: torch.Tensor, 
                        length_cache: int, 
                        length_max: int) -> torch.Tensor:

    threshold = length_max - length_cache
    penalty = torch.zeros_like(response_len, dtype=torch.float32)
    
    # if threshold < |y| <= length_max
    mask2 = (response_len > threshold) & (response_len <= length_max)
    penalty[mask2] = (threshold - response_len[mask2]) / length_cache
    
    # if |y| > length_max
    mask3 = response_len > length_max
    penalty[mask3] = -1.0
    
    # if |y| <= threshold, without penalty
...
class DAPOTrainer:
    ...
    def rollout(
        ...
        *,
        ...
        response_len = action_mask.sum(dim=-1)
    ...
    # ------------------------------------------------------------------
    # Step 3 · DAPO update: PPO-clip actor loss + KL penalty (no critic)
    # ------------------------------------------------------------------
    def dapo_update(self, experience: dict) -> dict:
        """
    ...
    def train(self):
        ...
        print(f"Start DAPO training for {args.num_train_epochs} epoch(s).")
        ...
        for epoch in range(1, args.num_train_epochs + 1):
            ...
            for batch in self.dataloader:
                ...
                # ── DAPO uses Soft Overlong Punishment ──
                rewards_shaped = dapo_length_penalty(rewards, response_len, args.length_cache, args.length_max)

                # ── Step 2: Make experience (group-relative advantages) ──
                experience = self.make_experience(rollout_data, rewards_shaped)

                # ── Step 3: DAPO update ──
                update_stats = self.dapo_update(experience)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Simple DAPO training for LLM")
    # paths
    parser.add_argument("--model_path", type=str, required=True, help="Path to pretrained model.")
    parser.add_argument("--train_json", type=str, required=True, help="Path to training json file.")
    parser.add_argument("--output_dir", type=str, default="output_dir/dapo")
    # generation
    ...
    parser.add_argument("--clip_eps_low", type=float, default=0.2, help="DAPO-clip epsilon.")
    parser.add_argument("--clip_eps_high", type=float, default=0.28, help="DAPO-clip epsilon.")
    parser.add_argument("--kl_coef", type=float, default=0.05, help="KL penalty coefficient (ref model).")
    parser.add_argument("--entropy_coef", type=float, default=0, help="Entropy loss coefficient. Default 0 for DAPO.")
    parser.add_argument("--length_cache", type=float, default=2048, help="DAPO Soft Overlong Punishment.")
    parser.add_argument("--length_max", type=float, default=8192, help="DAPO Soft Overlong Punishment.")
    # optimizer
