...
- 对于 $$L_{ppo}$$ ，其为RLHF中的标准形式， $$\mathbb{D}_{KL}$$ 为策略模型
- $$\mathbb{D}_{KL}$$ 为策略模型 $$\pi_{\theta}(y \mid x)$$ 与初始模型 $$\pi_{ref}(y \mid x)$$ 间的KL散度，整体优化目标为最大化奖励，同时限制 $$\pi_{\theta}(y \mid x)$$ 不要偏离初始模型过多。
